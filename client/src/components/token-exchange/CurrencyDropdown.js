        ...
        return (
            ...
            <div id="currency_droplist_from" className="cl-droplist  searchable sc-gqjmRU gywoMT">
                ...
                <ul className="sc-htpNat sc-fjdhpX bZoPFN">
                    ...
                    return(<li onClick={()=>this.onSelect(key)} className="sc-ifAKCX sc-cSHVUG ZfCCL"><button className="sc-kAzzGY cCVEey" type="button" tabindex="0"><div className="coin-list-item"><div className="coin-info"><i className="coin-icon"><img height="24" width="24" src={TokenLogo(key)}/></i><span className="coin-name" style={{fontWeight:'normal'}}><span className="coin-ticker" style={{fontWeight:'bolder',fontSize:'15px'}}>{currency.SYMBOL}</span>{currency.NAME}</span></div><div className="icons"><span className="fixed-rate-status">-</span></div></div></button></li>)
                    })}
